...
public class PdfGeneratorService {
    ...
    public String generateInvoicePdf(Invoice invoice) throws Exception {
        ...
        try (PdfWriter writer = new PdfWriter(new FileOutputStream(pdfPath));
            ...
            sellerTable.addCell(new Cell().add(new Paragraph("Seller: " + invoice.getSellerName()).setBold())
                    .setBorder(null).setTextAlignment(TextAlignment.LEFT));
            sellerTable.addCell(new Cell().add(new Paragraph("Website: " + invoice.getWebsite()).setBold())
                    .setBorder(null).setTextAlignment(TextAlignment.RIGHT));
            sellerTable.addCell(new Cell().add(new Paragraph("Phone: " + invoice.getSellerPhone()))
                    .setBorder(null).setTextAlignment(TextAlignment.LEFT));
            sellerTable.addCell(new Cell().add(new Paragraph("Address: " + invoice.getSellerAddress()))
                    .setBorder(null).setTextAlignment(TextAlignment.RIGHT));
            ...
            if (invoice.getCustomer() != null) {
                ...
                customerInfo += "Customer Name: " + invoice.getCustomer().getFirstName() + " " + invoice.getCustomer().getLastName() + "\n";
                customerInfo += "Phone: " + invoice.getCustomer().getPhone() + "\n";
                customerInfo += "Email: " + invoice.getCustomer().getEmail() + "\n";
            }
            customerAndAddressTable.addCell(new Cell().add(new Paragraph(customerInfo).setBold())
                    .setBorder(null).setTextAlignment(TextAlignment.LEFT));
            ...
            if (deliveryAddress != null) {
                ...
                deliveryInfo += deliveryAddress.getCity() + ", " + deliveryAddress.getState() + " " + deliveryAddress.getZipCode() + "\n";
                deliveryInfo += deliveryAddress.getCountry();
            }
            customerAndAddressTable.addCell(new Cell().add(new Paragraph(deliveryInfo).setBold())
                    .setBorder(null).setTextAlignment(TextAlignment.RIGHT));
            ...
            if (invoice.getOrder() != null) {
                ...
                document.add(new Paragraph("Order Date: " + invoice.getOrder().getOrderDate().format(formatter)));
            }
            ...
            document.add(new Paragraph("Payment Method: " + invoice.getPaymentMethod())
                    .setBold().setFontSize(12).setMarginBottom(15)); // Chữ in đậm
            ...
            for (InvoiceDetail detail : details) {
                ...
                table.addCell(new Cell().add(new Paragraph(product != null ? product.getName() : "Unknown Product"))
                        .setTextAlignment(TextAlignment.CENTER));
